import { Agreement } from "@prisma/client";
import moment from "moment";
import Link from "next/link";
import { useIPFS } from "../../providers/IPFSProvider";
import Badge from "../common/Badge";

export default function AgreementDetail({
  agreement,
}: {
  agreement: Agreement;
}) {
  console.log("agreement", agreement);

  const signersDisplay= Object.keys(agreement?.description || {}).map((key) => (
    <li key={key}>
      <Link href={`/agreements/${key}`}>
        <a
          className="m-2 flex items-center gap-2 rounded p-4 outline outline-dashed outline-1 outline-purple-200 hover:bg-purple-50"
          href="#"
        >
          <section className="basis-1/2">
            <p className="font-semibold">Signer Field</p>
            <p>{key}</p>
            <Badge
              className="w-36 bg-slate-900"
              text={key ? "Signed" : "Not signed"}
            />
          </section>
          <section className="flex grow basis-1/5 flex-wrap justify-start gap-1">
            {/* {Object.keys(key  || {}).map((key) => (
              <Badge text={key} key={key} />
            ))} */}
          </section>
          <section>
            <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg"> <path d="M8 2C7.44772 2 7 2.44772 7 3C7 3.55228 7.44772 4 8 4H10C10.5523 4 11 3.55228 11 3C11 2.44772 10.5523 2 10 2H8Z" fill="#4A5568"/> <path d="M3 5C3 3.89543 3.89543 3 5 3C5 4.65685 6.34315 6 8 6H10C11.6569 6 13 4.65685 13 3C14.1046 3 15 3.89543 15 5V11H10.4142L11.7071 9.70711C12.0976 9.31658 12.0976 8.68342 11.7071 8.29289C11.3166 7.90237 10.6834 7.90237 10.2929 8.29289L7.29289 11.2929C6.90237 11.6834 6.90237 12.3166 7.29289 12.7071L10.2929 15.7071C10.6834 16.0976 11.3166 16.0976 11.7071 15.7071C12.0976 15.3166 12.0976 14.6834 11.7071 14.2929L10.4142 13H15V16C15 17.1046 14.1046 18 13 18H5C3.89543 18 3 17.1046 3 16V5Z" fill="#4A5568"/> <path d="M15 11H17C17.5523 11 18 11.4477 18 12C18 12.5523 17.5523 13 17 13H15V11Z" fill="#4A5568"/> </svg>
          </section>
        </a>
      </Link>
    </li>
  ));
  const explorersDisplay= [
    "https://solscan.io/","https://solanabeach.io/"
  ].map((key) => (
    <li key={key}>
      <Link href={key}>
        <a
          className="m-2 flex items-center gap-2 rounded p-4 outline outline-dashed outline-1 outline-purple-200 hover:bg-purple-50"
          href={key}
          target="_blank_s"
        >
          <section className="basis-1/2">
            <p className="font-semibold">{key}</p>
          </section>
          <section>
            <Badge
              className="w-36 bg-slate-900"
              text="View"
            />
          </section>
        </a>
      </Link>
    </li>
  ));

  const pdfDisplay = (
    <iframe width="100%" src={`https://ipfs.io/ipfs/${agreement.cid}`}/>
  );

  return ( 
    <div className="grid grid-cols-2 gap-4">     
      <div>
        <p>Requested signatures: </p>
        <ul>{signersDisplay}</ul>
      </div>
      {pdfDisplay}
      <div>
        <p>Explorers:</p>
        <ul>{explorersDisplay}</ul>
      </div>
    </div>
  );
}
